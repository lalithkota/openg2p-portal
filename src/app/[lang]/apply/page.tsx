...
//                     <h1 className='font-fontcustom text-gray-700 pl-4 pb-4'>{forms[0].program_name}</h1>
...
//                     <p className=" text-gray-700 pt-4 pl-4 pb-0 font-fontcustom  ">{forms[0].program_name}</p>
...
export default function Apply() {
  ...
  Formio.use(tailwind);
  Templates.framework = "tailwind";
  const [form, setForm] = useState<ProgramForm | null>();
  const searchParams = useSearchParams()
  const programid = searchParams.get('programid')
  useEffect(() => {
    const fetchData = async () => {
      const formData = await fetchProgramForm(Number(programid));
      console.log(formData)
      if(formData?.schema){
        const formioElement = document.getElementById('formio');

        // Ensure the formioElement exists before creating the form
        if (formioElement) {
          Formio.createForm(formioElement, formData.schema).then(function (form) {
