...
// export default function ViewPage({ searchParams, params: { lang } }: {
//   searchParams?: {
//     query?: string;
//     page?: string;
//   },
...
//   //       const allApplications = await fetchApplicationDetails();

//   //       const selectedProgram = allPrograms.find(p => p.id === Number(programId));
//   //       const programApplications = allApplications.filter(app => app.program_name === selectedProgram?.name);

//   //       setApplications(programApplications);
//   //       setTotalPages(Math.ceil(programApplications.length / ITEMS_PER_PAGE));
//   //     } catch (error) {
...
//   // }, [lang, programId]);
//   useEffect(() => {
//     const fetchAndFilterApplications = async (programid: string) => {
//         setIsLoading(true);
//         try {
//             const allPrograms = await fetchPrograms();
//             const allApplications = await fetchApplicationDetails();
//             const selectedProgram = allPrograms.find(program => program.id.toString() === programid);

//             if (selectedProgram) {
//                 const filteredApplications = allApplications.filter(application => application.program_id === selectedProgram.id);
...
//         if (programid) {
//             fetchAndFilterApplications(programid);
//         }
//     }
// }, [router, lang]);
...
//     setPaginatedApplications(applications.slice(start, end)); // Change from 'setApplications' to 'setPaginatedApplications'
...
//   if (!applications.length) return <div>No applications found for this program.</div>;
